...
def main(argv=sys.argv):
    ...
    def pause_all_robots(request, response):
        del request
        count = 0
        for robot in robots.values():
            if robot.pause_for_tutorial():
                count += 1
        response.success = count > 0
        response.message = f'Paused {count} robot(s)'
        return response

    def resume_all_robots(request, response):
        del request
        count = 0
        for robot in robots.values():
            if robot.resume_for_tutorial():
                count += 1
        response.success = count > 0
        response.message = f'Resumed {count} robot(s)'
        return response

    def reset_all_robots(request, response):
        del request
        count = 0
        for robot in robots.values():
            if robot.reset_for_tutorial():
                count += 1
        response.success = count > 0
        response.message = f'Reset {count} robot(s)'
        return response

    node.create_service(Trigger, '/simple_pause_robots', pause_all_robots)
    node.create_service(Trigger, '/simple_resume_robots', resume_all_robots)
    node.create_service(Trigger, '/simple_reset_robots', reset_all_robots)
...
class RobotAdapter:
    ...
    def navigate(self, destination, execution):
        ...
        if not self.api.navigate(
            ...
            self.node.get_logger().warn(
                f'Failed to command [{self.name}] to navigate to '
                f'{destination.position} on map [{destination.map}]. '
                f'Requesting replanning...'
            )
            self.execution = None
            self.replan()
    ...
    def stop(self, activity):
        ...
        if execution is not None:
            if execution.identifier.is_same(activity):
                self.execution = None
                if not self.api.stop(self.name):
                    self.node.get_logger().warn(
                        f'Failed to stop [{self.name}]'
                    )

    def pause_for_tutorial(self):
        self.node.get_logger().warn(
            f'Tutorial pause requested for [{self.name}]'
        )
        return self.api.pause(self.name)

    def resume_for_tutorial(self):
        self.node.get_logger().warn(
            f'Tutorial resume requested for [{self.name}]'
        )
        return self.api.resume(self.name)

    def reset_for_tutorial(self):
        self.node.get_logger().warn(
            f'Tutorial reset requested for [{self.name}]'
        )
        self.execution = None
        return self.api.reset(self.name)
    ...
    def execute_action(self, category: str, description: dict, execution):
        ...
        if isinstance(description, dict):
            label = description.get('label') or category

        self.node.get_logger().info(
            f'Commanding [{self.name}] to perform action [{category}] '
            f'with label [{label}]'
        )

        if not self.api.start_activity(self.name, category, label):
            self.node.get_logger().warn(
                f'Failed to start action [{category}] for [{self.name}]. '
                f'Requesting replanning...'
            )
            self.execution = None
            self.replan()
